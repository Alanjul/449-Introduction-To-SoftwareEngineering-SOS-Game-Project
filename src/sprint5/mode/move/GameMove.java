...
public class GameMove {
     ...
     */
    public GameMove(Integer gameId, int moveNumber, Integer playerId,
                    int rowPosition, int colPosition, char letter,
                    int sosFormed, int pointsScored) {
        this(null, gameId, moveNumber, playerId, rowPosition, colPosition,
             letter, sosFormed, pointsScored, LocalDateTime.now());
    }
    
    /**
     * Full constructor (for loading from database).
     */
    public GameMove(Integer moveId, Integer gameId, int moveNumber, Integer playerId,
                    int rowPosition, int colPosition, char letter,
                    int sosFormed, int pointsScored, LocalDateTime moveTimestamp) {
    ...
    private void validateGameId(Integer gameId) {
        if (gameId == null) throw new IllegalArgumentException("Game ID cannot be null");
    }
    ...
    public int getSosFormed() { return sosFormed; }
    public int getPointsScored() { return pointsScored; }
    public LocalDateTime getMoveTimestamp() { return moveTimestamp; }
    
    // Setter
    public void setMoveId(Integer moveId) {
        if (this.moveId != null) throw new IllegalStateException("Move ID already set");
        this.moveId = moveId;
    }
    
    // Helper
    public boolean didScore() { return sosFormed > 0 || pointsScored > 0; }
    ...
    public boolean equals(Object o) {
        ...
        return Objects.equals(moveId, that.moveId) &&
    ...
    public String toString() {
        return String.format("GameMove[#%d: %c at (%d,%d), SOS=%d, Pts=%d]",
            moveNumber, letter, rowPosition, colPosition, sosFormed, pointsScored);
    }
