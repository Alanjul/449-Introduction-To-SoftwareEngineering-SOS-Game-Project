...
public class StatsRecord {
    ...
    public StatsRecord(Integer statsId, Integer playerId, 
            int gamesPlayed, int gamesWon, int gamesLost, int gamesDrawn,
            int totalSosFormed, int totalPointsScored, 
            LocalDateTime lastUpdated) {
validatePlayerId(playerId);
validateStats(gamesPlayed, gamesWon, gamesLost, gamesDrawn, 
           totalSosFormed, totalPointsScored);
...
this.lastUpdated = lastUpdated != null ? lastUpdated : LocalDateTime.now();
    ...
    private void validateStats(int played, int won, int lost, int drawn, int sos, int points) {
        ...
        if (played != (won + lost + drawn)) {
            throw new IllegalArgumentException(
                String.format("Games played (%d) must equal won + lost + drawn (%d)",
  ...
  //Getters
    ...
    public int getTotalSosFormed() { return totalSosFormed; }
    ...
    public void addSosFormed(int sosCount) {
        if (sosCount < 0) throw new IllegalArgumentException("SOS count cannot be negative");
    ...
    public boolean equals(Object o) {
        ...
        return Objects.equals(statsId, that.statsId) &&
    ...
    public String toString() {
        return String.format("StatsRecord[played=%d, won=%d, lost=%d, drawn=%d, winRate=%.1f%%]",
            gamesPlayed, gamesWon, gamesLost, gamesDrawn, getWinRate());
    } 
