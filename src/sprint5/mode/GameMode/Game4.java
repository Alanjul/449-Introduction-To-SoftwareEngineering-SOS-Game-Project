...
public class Game4 {
	...
	protected Game4(Board board, Player bluePlayer, Player redPlayer, GameMode4 mode, PatternDetector patternDetector,
			GameOverStrategy gameOverStrategy) {
		if (board == null || bluePlayer == null || redPlayer == null || mode == null || patternDetector == null
				|| gameOverStrategy == null) {
	...
	public MoveResult makeMove(Move move) {
		...
		List<LineSegment4> newLines = patternDetector.findPatterns(board, move.getRow(), move.getCol(),
	...
	public GameResult getResult() {
		...
		return new GameResult(state, message, scoreManager.getBlueScore(), scoreManager.getRedScore(), winner);
	}
	...
	private Winner determineWinner() {
		...
		return Winner.fromScores(scoreManager.getBlueScore(), scoreManager.getRedScore(), true);
	}

	private String generateResultMessage() {
		if (state == GameState.IN_PROGRESS) {
			return String.format("%s's turn in %s mode - Blue: %d, Red: %d", getCurrentPlayer().getName(),
					mode.getName(), scoreManager.getBlueScore(), scoreManager.getRedScore());
		}
		return switch (state) {
		case BLUE_PLAYER_WINS -> String.format("Blue Player wins! Final score: Blue %d - Red %d",
				scoreManager.getBlueScore(), scoreManager.getRedScore());
		case RED_PLAYER_WINS -> String.format("Red Player wins! Final score: Blue %d - Red %d",
				scoreManager.getBlueScore(), scoreManager.getRedScore());
		case DRAW -> String.format("Draw! Final score: Blue %d - Red %d", scoreManager.getBlueScore(),
