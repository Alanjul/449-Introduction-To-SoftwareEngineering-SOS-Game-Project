...
public class GameReplayService {
    ...
    public Optional<GameReplay> loadReplay(Integer gameId) {
        // Get game
       try { Optional<GameRecord> gameOpt = gameDAO.findById(gameId);
    ...
    public List<GameReplay> getPlayerReplays(Integer playerId) {
        try{List<GameRecord> games = gameDAO.getPlayerGames(playerId);
        return loadMultipleReplays(games);
        } catch(Exception e)
        {
    ...
    public List<GameReplay> getReplaysBetweenPlayers(Integer player1Id, Integer player2Id) {
       try { List<GameRecord> games = gameDAO.getGamesBetweenPlayers(player1Id, player2Id);
        return loadMultipleReplays(games.stream()
            .filter(GameRecord::isCompleted)
    ...
    public List<GameReplay> getReplaysByMode(String gameMode) {
        try {List<GameRecord> games = gameDAO.getGamesByMode(gameMode);
        return loadMultipleReplays(games.stream()
            .filter(GameRecord::isCompleted)
            .collect(java.util.stream.Collectors.toList()));
        } catch(Exception e)
    ...
    public List<GameReplay> getReplaysByDateRange(LocalDateTime start, LocalDateTime end) {
       try { List<GameRecord> games = gameDAO.getGamesByDateRange(start, end);
        return loadMultipleReplays(games.stream()
            .filter(GameRecord::isCompleted)
            .collect(java.util.stream.Collectors.toList()));
       } catch (Exception e)
    ...
    public static class ReplayStatistics {
        ...
        public double getAverageMoveTime() {
            if (replay.getTotalMoves() == 0) return 0.0;
            return (double) replay.getDurationSeconds() / replay.getTotalMoves();
        }
        
        public int getTotalSOSFormed() {
            return replay.getMoves().stream()
                .mapToInt(GameMove::getSosFormed)
                .sum();
        }
    ...
    public String toString() {
        return String.format(
            "Moves: %d | Duration: %s | Score: %d-%d | Winner: %s | Total SOS: %d",
            getTotalMoves(),
    ...
    public static class ReplayModeCounts {
        ...
        @Override
        public String toString() {
            return String.format("Simple: %d | General: %d | Total: %d",
                simpleCount, generalCount, getTotalCount());
        }
    }
