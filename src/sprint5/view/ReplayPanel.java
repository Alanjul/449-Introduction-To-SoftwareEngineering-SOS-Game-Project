...
public class ReplayPanel extends JPanel {
    ...
    private void updateSpeed() {
        ...
        if (selected.contains("Slow")) controller.setSpeedPreset("SLOW");
        else if (selected.contains("Normal")) controller.setSpeedPreset("NORMAL");
        else if (selected.contains("Fast")) controller.setSpeedPreset("FAST");
    ...
    private void togglePlayPause() {
        ...
        if (controller.isPlaying()) playTimer.start();
        else playTimer.stop();
  ...
  //REF @ChatGPT
    private JPanel createGameInfoPanel() {
        ...
        gameInfoLabel = new JLabel(
            String.format("<html><b>Game #%d:</b> %s (Blue) vs %s (Red) | Mode: %s | Board: %dx%d</html>",
                replay.getGameId(), replay.getBluePlayerName(), replay.getRedPlayerName(),
                replay.getGameMode(), replay.getBoardSize(), replay.getBoardSize())
        );
    ...
    private void applyMove(GameMove move) {
        ...
        Color color = move.getPlayerId().equals(controller.getReplay().getBluePlayerId())
    ...
    private void replayAllMoves() {
        ...
        for (int i = 0; i <=currentIndex && i <replay.getTotalMoves(); i++) {
            ...
            Color color = move.getPlayerId().equals(replay.getBluePlayerId())
                    ? Color.BLUE : Color.RED;
            board.makeMove(move.getRowPosition(), move.getColPosition(), move.getLetter());
    ...
    private void refreshReplayUI() {
        ...
        moveInfoLabel.setText(String.format(
            "Move: %d / %d (%.0f%%)",
            controller.getCurrentMoveIndex(),
            controller.getTotalMoves(),
        ...
        if (controller.getCurrentMoveIndex() == 0) {
            ...
            statusLabel.setText(String.format(
                "Replay completed - %s (Blue: %d, Red: %d)",
                winnerText,
                replay.getBlueScore(),
                replay.getRedScore()
            ));
            ...
            GameMove move = controller.getCurrentMove();
            if (move != null) {
                String player = move.getPlayerId().equals(controller.getReplay().getBluePlayerId())
                        ? "Blue" : "Red";

                statusLabel.setText(String.format(
                    "Move %d: %s played %c at (%d, %d) - SOS: %d",
                    controller.getCurrentMoveNumber(),
                    player,
                    move.getLetter(),
                    move.getRowPosition(),
                    move.getColPosition(),
