...
public class SOSGameController {
	...
	public SOSGameController(GameFrame frame, ComputerStrategy strategy) {
		...
		this.persistence = new GamePersistenceService(daos.getPlayerDAO(), daos.getGameDAO(), daos.getStatsDAO());
	...
	public SOSGameController(GameFrame frame, Map<GameMode4, GameFactory> gameInterfaces,
			Map<Boolean, PlayerFactory> playerInterfaces, GamePersistenceService persistence,
	...
	private void startNewGame() {
		...
		try {
			...
			bluePlayer = createPlayer('B', panel.getBluePlayerPanel().isHuman(),
					panel.getBluePlayerPanel().getDifficultyEnum(), panel.getBluePlayerPanel().getSelectedLetter());

			redPlayer = createPlayer('R', panel.getRedPlayerPanel().isHuman(),
					panel.getRedPlayerPanel().getDifficultyEnum(), panel.getRedPlayerPanel().getSelectedLetter());
			...
			if (panel.getBottomPanel().isRecordGameEnabled()) {
				...
				currentSession = persistence.startGameSession(bluePlayer.getName(), redPlayer.getName(), mode,
						boardSize, bluePlayer.isHumanPlayer(), redPlayer.isHumanPlayer());
	...
	private Player createPlayer(char symbol, boolean isHuman, ComputerPlayer.LevelsOfDifficulty difficulty,
		...
		if (factory == null) {
			throw new IllegalStateException("No factory for player type: " + (isHuman ? "Human" : "Computer"));
		}
	...
	private void makeMove(int row, int col, char letter) {
		try {
			...
			if (result.getSosCount() > 0) {
				statistics.recordSOS(currentPlayer, result.getSosCount(), result.getSosCount());
			}

			if (currentSession != null) {
				Integer playerId = (currentPlayer.getSymbol() == 'B') ? currentSession.getBluePlayerId()
						: currentSession.getRedPlayerId();

				persistence.recordMove(currentSession.getGameId(), moveCounter, playerId, row, col, letter,
						result.getSosCount(), result.getSosCount());
			}
			...
			if (game.isGameOver()) {
				...
				updateStatusLabel("Current turn: " + (game.getCurrentPlayer().getSymbol() == 'B' ? "Blue" : "Red"));
				if (game.getCurrentPlayer() instanceof ComputerPlayer) {
					scheduleComputerMove();
				}
			}
	...
	private void handleGameOver() {
		...
		if (currentSession != null) {
			try {
				persistence.completeGame(currentSession, result.getWinner(), game.getBluePlayer().getScore(),
						game.getRedPlayer().getScore(), statistics.getBlueStats(), statistics.getRedStats());
	...
	public GameStatistics getStatisticsManager() {
		return statistics;
	}
