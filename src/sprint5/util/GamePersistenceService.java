...
public class GamePersistenceService 
{
	...
	// Starts Game Session
	public GameSession startGameSession(String bluePlayerName, String redPlayerName, String gameMode, int boardSize,
			boolean blueIsHuman, boolean redIsHuman)
	{
		try {
			 String uniqueBluePlayerName = bluePlayerName;
	            ...
	            throw new RuntimeException("Failed to start game session", e);
	        }
	}
	...
	private PlayerRecord loadOrCreatePlayer(String playerName, boolean isHuman) {
        ...
        Integer playerId = playerDAO.create(newPlayer);
	...
	public void recordMove(Integer gameId, int moveNumber, Integer playerId, int row, int col, char letter,
		...
		try {
			GameMove move = new GameMove(gameId,
	                moveNumber,
	                playerId,
	                row,
	                col,
	...
	public void completeGame(GameSession session, Winner winner, int blueScore, int redScore,
		...
		try {
			...
			updatePlayerStatistics(session.getBluePlayerId(), blueSessionStats, winner == winner.BLUE, winner == Winner.DRAW);

			updatePlayerStatistics(session.getRedPlayerId(), redSessionStats, winner == Winner.RED, winner == Winner.DRAW);
			...
			throw new RuntimeException("Failed to complete game", e);
		}
	}

	/*
	 * Update player statistics in databasees
	 */
	private void updatePlayerStatistics(Integer playerId,  PlayerStats sessionStats,
                    boolean won, boolean draw) 
	{
		try {
			...
			statsDAO.updateAfterGame(
			playerId,
			gamesWon,
			gamesLost,
			gamesDrawn,
			sessionStats.getTotalSOSFormed(),
	...
	// Create Stats for a Player
	 public StatsRecord loadOrCreateStats(Integer playerId) {
	        return statsDAO.findByPlayerId(playerId)
	            .orElseGet(() -> {
